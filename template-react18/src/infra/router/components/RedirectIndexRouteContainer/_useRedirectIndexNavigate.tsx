import { type CustomRouteObject, matchRoutes, Navigate, useLocation } from 'react-router';
import { useMemo } from 'react';

/**
 * 인덱스 route 또는 첫 번째 child route로 이동시키는 custom hook
 */
const useRedirectIndexNavigate = (routes: CustomRouteObject[]) => {
  const location = useLocation();

  const redirectPath = useMemo(() => {
    if (!routes) return undefined; // 전체 라우트가 없다면

    const matchedRoutes = matchRoutes(routes, location); // 전체 routes 비교
